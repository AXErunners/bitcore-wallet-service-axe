...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb, isInstantSend) {
  ...
  self._getUtxosForCurrentWallet({}, function(err, utxos) {
    ...
    // Sanitizing our utxo array to exclude transactions with fewer than 6 confirmations will remove all available inputs
    if (isInstantSend && (utxos === undefined || utxos.length == 0)) return cb(Errors.INSUFFICIENT_CONFIRMATIONS);
        ...
        async.series([
          ...
          function(next){
            if(opts.isInstantSend===true){
                ...
                var feePerInput = 10000;//Fee for IS are 0.0001 * INPUTS
...
WalletService.prototype._broadcastixRawTx = function(coin, network, raw, cb) {
    var bc = this._getBlockchainExplorer(coin, network);
    if (!bc) return cb(new Error('Could not get blockchain explorer instance'));
 ...
 * @param {string} [opts.network = 'livenet'] - The AXE network for this transaction.
...
WalletService.prototype.broadcastIXRawTx = function(opts, cb){
    ...
    self._broadcastixRawTx(opts.coin, opts.network, opts.rawTx, cb);
};
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.getTx({
      ...
      if(txp.hasOwnProperty('isInstantSend') && txp.isInstantSend===true){
        ...
        self._broadcastixRawTx(wallet.coin, wallet.network, raw, function(err, txid) {
          if (err) {
